...
class SheetRef {
  ...
  final String? path;
  const SheetRef({required this.id, required this.title, required this.createdAt, this.path});
}
...
class _SheetsBrowserScreenState extends State<SheetsBrowserScreen>
  ...
  /// Menú de creación: Medición clásica (app principal) o Bloc de notas (Pluto)
  Future<void> _createAndOpen() async {
    ...
    switch (choice) {
      case 'measure': {
        ...
        final ref = SheetRef(id: meta.id, title: meta.name, createdAt: meta.createdAt.toLocal());
        await _open(ref);
        break;
      }
      case 'pluto': {
        ...
        await Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => NoteSheetPlutoScreen(id: d.id, theme: widget.theme)),
        );
        await _load();
        break;
      }
                              ...
                              return SizedBox(
                                ...
                                child: GridView.builder(
                                  ...
                                  itemBuilder: (_, i) {
                                    ...
                                    final now = DateTime.now();
                                    final today = now.year == k.year && now.month == k.month && now.day == k.day;
...
class _CountDot extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Container(width: 8, height: 8, decoration: const BoxDecoration(shape: BoxShape.circle, color: Colors.white));
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: items.isEmpty
        ...
        child: Text('Sin planillas en este día', style: TextStyle(color: t.text.withValues(alpha: .7))),
      )
          : ListView.separated(
