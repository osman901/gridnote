// IA local ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œExcel-likeÃƒÂ¢Ã¢â€šÂ¬Ã‚Â: autocompleta, valida y aprende con Hive.
...
class EliteAssistant implements GridnoteAssistant {
  ...
  /// Crea una instancia para una hoja concreta.
  /// Guardamos el sheetId para posibles comportamientos por hoja en el futuro.
  ...
  Future<AiResult> transform(AiCellContext ctx) async {
    ...
    // --- NÃƒÆ’Ã…Â¡MEROS (ohm1m / ohm3m) ---
    if (col == 'ohm1m' || col == 'ohm3m') {
      final norm = (ctx.rawInput ?? '').toString().replaceAll(',', '.');
      final v = double.tryParse(norm);
      if (v == null) return AiResult.reject('NÃƒÆ’Ã‚Âºmero invÃƒÆ’Ã‚Â¡lido');

      final clamped = v.clamp(0.0, 999999.0).toDouble();
      final hint =
          'Promedio histÃƒÆ’Ã‚Â³rico: ${_mean(col, clamped).toStringAsFixed(2)}';
    ...
    if (col == 'date') {
      ...
      return (d == null)
          ? AiResult.reject('Fecha invÃƒÆ’Ã‚Â¡lida')
          : AiResult.accept(d);
    }
