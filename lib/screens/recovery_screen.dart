...
class _RecoveryScreenState extends State<RecoveryScreen> {
  final _daysOptions = const [7, 14, 30];
  int _days = 14;
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      List<dynamic> raw =
      await (reg.recentlyDeleted?.call(days: _days) ??
  ...
  Widget build(BuildContext context) {
    final theme = GridnoteThemeController().theme;
          ...
          Expanded(
            child: _loading
              ...
              itemBuilder: (_, i) {
                final it = _items[i];
                final when =
                    '${it.deletedAt.day.toString().padLeft(2, '0')}/${it.deletedAt.month.toString().padLeft(2, '0')} ${it.deletedAt.hour.toString().padLeft(2, '0')}:${it.deletedAt.minute.toString().padLeft(2, '0')}';
                return ListTile(
...
class _TrashItem {
  ...
  const _TrashItem({required this.id, required this.title, required this.deletedAt});
}
