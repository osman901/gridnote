...
class _ConfiguracionScreenState extends State<ConfiguracionScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (await jf.exists()) {
        ...
        // MigraciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n simple desde .txt (legacy)
        ...
        if (await tf.exists()) {
          try {
            ...
            // Guarda inmediatamente en JSON y podÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©s borrar el legacy si querÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©s
            await _guardarInfo(silent: true);
            // await tf.delete(); // opcional
          } catch (e) {
            debugPrint('MigraciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n legacy fallÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³: $e');
          }
        }
      }
    } catch (e) {
      debugPrint('Error al cargar configuraciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Error al cargar la configuraciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n.')),
        );
      }
    } finally {
  ...
  Future<void> _guardarInfo({bool silent = false}) async {
    ...
    try {
      ...
      // Escritura atÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³mica: rename sobre el destino
      if (await jf.exists()) {
        try {
          await jf.delete();
        } catch (_) {}
      ...
      if (!silent && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡Datos guardados!')),
        );
      }
    } catch (e) {
      debugPrint('Error al guardar la informaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n: $e');
      if (!silent && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
  ...
  String? _emailValidator(String? v) {
    final s = (v ?? '').trim();
    if (s.isEmpty) return null; // opcional: permitir vacÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­o
    final ok = RegExp(r'^[^@\s]+@[^@\s]+\.[^@\s]+$').hasMatch(s);
    return ok ? null : 'Email invÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡lido';
    // Si querÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©s hacerlo requerido:
    // return s.isEmpty ? 'Requerido' : (ok ? null : 'Email invÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡lido');
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('ConfiguraciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n'),
        actions: [
          IconButton(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  TextFormField(
                    ...
                    decoration: const InputDecoration(
                      labelText: 'DirecciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n',
                      border: OutlineInputBorder(),
                    ),
                    enabled: !disabled,
                    maxLines: 2,
...
class _ColorPickerDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    var current = color;
    return AlertDialog(
      title: const Text('ElegÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­ el color'),
