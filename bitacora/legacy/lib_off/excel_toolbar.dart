...
class _ExcelToolbarState extends State<ExcelToolbar> {
  ...
  Future<void> _pickColor() async {
    ...
    await widget.gridController.colorCellSelected(
      picked.withValues(alpha: 0.45),
    );
  }
  ...
  Future<void> _setMyLocation() async {
    if (!await Geolocator.isLocationServiceEnabled()) {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Activá el GPS')));
      return;
    }
    ...
    if (perm == LocationPermission.denied ||
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Sin permiso de ubicación')));
      return;
    }

    // API moderna (sin deprecation):
    final pos = await LocationService.instance.currentPosition(
      accuracy: LocationAccuracy.best,
    );

    await widget.gridController.setLocationOnSelection(
      pos.latitude,
      pos.longitude,
    );
