...
class SheetViewModel {
  ...
  final ValueNotifier<Map<String, String>> headers =
  ValueNotifier(<String, String>{});
  final ValueNotifier<List<Measurement>> measurements =
  ValueNotifier(<Measurement>[]);
  ...
  Future<void> _loadHeaders() async {
    ...
    final map = (raw == null || raw.isEmpty)
        ? <String, String>{}
        : Map<String, String>.from(Uri.splitQueryString(raw));
    headers.value = map;
  }
  ...
  Future<void> saveTitle(String next) async {
    ...
    onSnack('TÃƒÆ’Ã‚Â­tulo guardado.');
  }
  ...
  bool _isInvalidPosition(Position? p) {
    ...
    final badAccuracy = p.accuracy.isFinite ? (p.accuracy > 1000) : false; // >1km
  ...
  Future<Position?> _tryGetCurrent({
    ...
    try {
      return await Geolocator.getCurrentPosition(
          desiredAccuracy: acc, timeLimit: timeout);
    } on TimeoutException {
      return null;
    } catch (_) {
      return null;
  ...
  Future<Position?> _getLastKnownRecent(
    ...
    try {
      ...
      switch (perm) {
        case PermResult.disabled:
          onSnack('ActivÃƒÆ’Ã‚Â¡ el GPS para guardar la ubicaciÃƒÆ’Ã‚Â³n.');
          return;
        case PermResult.denied:
          onSnack('Permiso de ubicaciÃƒÆ’Ã‚Â³n denegado.');
          return;
      ...
      // Estrategia robusta: actual ? lastKnown reciente ? intento adicional.
      ...
      if (_isInvalidPosition(pos)) {
        pos = await _tryGetCurrent(
            timeout: const Duration(seconds: 12),
            acc: LocationAccuracy.medium);
      }
      if (_isInvalidPosition(pos)) {
        onSnack('No se pudo obtener una ubicaciÃƒÆ’Ã‚Â³n vÃƒÆ’Ã‚Â¡lida.');
        return;
      }
      ...
      onSnack('UbicaciÃƒÆ’Ã‚Â³n guardada.');
      audit.log('location_saved', {'lat': lat.value, 'lng': lng.value});
    } catch (e, st) {
      _logError(e, st);
      onSnack('No se pudo obtener la ubicaciÃƒÆ’Ã‚Â³n.');
    } finally {
      isBusy.value = false;
    }
  }
  ...
  Future<void> openMapsFor({double? latParam, double? lngParam}) async {
    ...
    if (la == null || lo == null) {
      onSnack('UbicaciÃƒÆ’Ã‚Â³n no disponible.');
      return;
    }
    if (la.abs() < 1e-6 && lo.abs() < 1e-6) {
      onSnack('UbicaciÃƒÆ’Ã‚Â³n invÃƒÆ’Ã‚Â¡lida (0,0). IntentÃƒÆ’Ã‚Â¡ volver a guardarla.');
      return;
    }

    final bool isIOS = defaultTargetPlatform == TargetPlatform.iOS;
    final Uri uriApp =
    isIOS ? Uri.parse('maps://?q=$la,$lo') : Uri.parse('geo:$la,$lo?q=$la,$lo');
  ...
  Future<File> _makePdfFile(List<Measurement> rows) async {
    ...
    return PdfExportService.export(
        title: title.value, rows: rows, fileName: name);
  }
  ...
  String buildEmailBody(List<Measurement> rows) {
    ...
    if (la != null && lo != null) {
      b.writeln('');
      b.writeln('UbicaciÃƒÆ’Ã‚Â³n general:');
    ...
    if (withCoords.isNotEmpty) {
      ...
      for (final m in withCoords.take(10)) {
        ...
        b.writeln('ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ $prog ? $url');
      }
      if (withCoords.length > 10) {
        b.writeln('ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ (+${withCoords.length - 10} mÃƒÆ’Ã‚Â¡s)');
      }
    }
    return b.toString();
  }
  ...
  Future<void> shareViaEmailXlsx({
    ...
    try {
      ...
      if (!isEmailValid(trimmed)) {
        onSnack('Email invÃƒÆ’Ã‚Â¡lido. RevisÃƒÆ’Ã‚Â¡ el destinatario.');
        return;
      }
      ...
      try {
        ...
        onSnack('No se pudo guardar el archivo. VerificÃƒÆ’Ã‚Â¡ el espacio disponible.');
        return;
      }
      ...
      final mail = Email(
        recipients: [trimmed],
        subject: 'Gridnote ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ ${title.value}',
        body: bodyText,
      ...
      try {
        ...
        onSnack('Se abriÃƒÆ’Ã‚Â³ tu app de correo. TocÃƒÆ’Ã‚Â¡ ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œEnviarÃƒÂ¢Ã¢â€šÂ¬Ã‚Â.');
      } catch (_) {
        final mailto = Uri(
          scheme: 'mailto',
          path: trimmed,
  ...
  Future<void> shareCsv({required List<Measurement> rows}) async {
    ...
    try {
      ...
      await Share.shareXFiles([XFile(file.path, mimeType: 'text/csv')],
          subject: 'Gridnote ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ ${title.value}', text: buildEmailBody(rows));
    } finally {
      isBusy.value = false;
    }
  }
  ...
  List<Measurement> _importedRows(List<Measurement> rows) {
    ...
    _pushUndo();
    final next = List<Measurement>.from(measurements.value)
      ..addAll(rows.map(_withLocalId));
  ...
  // --- Datos / ediciÃƒÆ’Ã‚Â³n ---
  void addRow() {
    _pushUndo();
    // ?? FIX: si Measurement.empty es getter, no lo invoques.
    final next = List<Measurement>.from(measurements.value)
      ..add(_withLocalId(Measurement.empty));
    measurements.value = next;
    HapticFeedback.selectionClick();
    audit.log('row_add', {});
  }
  ...
  // --- BÃƒÆ’Ã‚Âºsqueda (debounce) ---
  void setQueryDebounced(String q,
  ...
  Measurement _withLocalId(Measurement m) =>
      (m.id != null) ? m : m.copyWith(id: _genTempId());
  List<Measurement> _ensureLocalIds(List<Measurement> list) =>
      list.map(_withLocalId).toList(growable: true);

  void _logError(Object e, [StackTrace? st]) {
    // TODO: Integra Crashlytics/Sentry si querÃƒÆ’Ã‚Â©s.
    // ignore: avoid_print
