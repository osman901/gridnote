...
class _EntriesScreenState extends ConsumerState<EntriesScreen> {
  ...
  Future<void> _setLocation(EntryRow e) async {
    ...
    if (perm == LocationPermission.denied || perm == LocationPermission.deniedForever) {
      if (mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('Permiso de ubicaciÃƒÆ’Ã‚Â³n denegado')));
      }
      return;
    }
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('UbicaciÃƒÆ’Ã‚Â³n guardada')));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: entriesAsync.when(
        data: (entries) {
          if (entries.isEmpty) {
            return const Center(child: Text('Sin filas aÃƒÆ’Ã‚Âºn. Crea la primera.'));
          }
                ...
                confirmDismiss: (_) async {
                  final ok = await showDialog<bool>(
                    ...
                    builder: (_) => AlertDialog(
                      ...
                      content: const Text('Ãƒâ€šÃ‚ÂSeguro que quieres eliminar esta fila?'),
                      actions: [
                          ...
                          children: [
                            if (e.lat != null && e.lng != null)
                              Chip(
                                ...
                                label: Text(
                                  '(${e.lat!.toStringAsFixed(5)}, ${e.lng!.toStringAsFixed(5)}) ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ ${e.provider ?? 'gps'}',
                                ),
                              )
                            else
                              const Text('Sin ubicaciÃƒÆ’Ã‚Â³n', style: TextStyle(color: Colors.grey)),
                            FilledButton.icon(
                              onPressed: () => _setLocation(e),
                              icon: const Icon(Icons.my_location),
                              label: const Text('Guardar ubicaciÃƒÆ’Ã‚Â³n'),
                            ),
                          ],
                        ),
                        ...
                        Row(
                          children: [
                            ...
                            countAsync.when(
                              ...
                              loading: () => const Text('Adjuntos: ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦'),
                              error: (_, __) => const Text('Adjuntos: error'),
                            ),
                            const Spacer(),
                            Text(
                              'ID ${e.id} ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ ${e.createdAt}',
