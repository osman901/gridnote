...
class FillSuggestion {
  ...
  final Measurement template; // mediciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“BÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â sugerida (sin id)
  final String reason;
}

/// Sugiere ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“BÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â si ve huecos AÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦C en progresivas (step=100) y estima Ohm por promedio de vecinos.
class FillSuggester {
  ...
  List<FillSuggestion> suggest(List<Measurement> items) {
    ...
    for (var i = 0; i + 1 < withP.length; i++) {
      ...
      if (pa.km == pc.km &&
          pc.plus - pa.plus >= step * 2 &&
          (pc.plus - pa.plus) % step == 0) {
        ...
        for (var j = 1; j <= missing; j++) {
          ...
          final neighbors = withP
              ...
              .sorted((x, y) {
            final dx =
            ((Progresiva.parse(x.progresiva)?.plus ?? 0) - pb.plus).abs();
            final dy =
            ((Progresiva.parse(y.progresiva)?.plus ?? 0) - pb.plus).abs();
          ...
          // ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã‚Â¡Ãƒâ€šÃ‚ ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â Evita invocar algo que no es funciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n: usar un ctor vÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡lido.
          final template = const Measurement(
            progresiva: '',
            observations: '',
          ).copyWith(
            date: DateTime.now(),
