...
import 'dart:async';
import 'dart:math' as math; // (queda por si lo usÃƒÆ’Ã‚Â¡s en otro lado)
...
class _MeasurementListScreenState extends State<MeasurementListScreen> {
  ...
  void initState() {
    ...
    _searchCtrl.addListener(() {
      ...
      _debounce = Timer(const Duration(milliseconds: 350), () {
        _applyFilter(); // Future ignorado a propÃƒÆ’Ã‚Â³sito
      });
    });
  }
  ...
  String _buildDashboard() {
    final list = _filtered
        ...
        .toList();
    if (list.length < 2) {
      return 'Mediciones: ${_filtered.length} ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ Distancia: 0 m';
    }
    // Orden seguro por fecha (soporta null)
    list.sort((a, b) {
    ...
    // Distancia con Geolocator (robusto y probado)
    final d = Geolocator.distanceBetween(
      list.first.latitude!, list.first.longitude!,
      list.last.latitude!, list.last.longitude!,
    );

    return 'Mediciones: ${_filtered.length} ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ Distancia: ${d.toStringAsFixed(0)} m';
  }
        ...
        actions: [
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (_) => [
              ...
              PopupMenuItem(
                value: 'dir',
                child: Text(_asc ? 'Ascendente' : 'Descendente'),
              ),
            ],
              ...
              itemBuilder: (ctx, i) {
                ...
                return Dismissible(
                  ...
                  child: ListTile(
                    ...
                    onTap: () {
                      if (_selectionMode) {
                        setState(() => selected ? _sel.remove(i) : _sel.add(i));
                      } else {
                        _openEditor(m);
                      }
                    },
      ...
      bottomNavigationBar: isSelecting
        ...
        child: Row(
          children: [
            ...
            const Spacer(),
            IconButton(
              tooltip: 'Exportar PDF selecciÃƒÆ’Ã‚Â³n',
              onPressed: _exportSelectedPdf,
              icon: const Icon(Icons.picture_as_pdf),
            ),
            IconButton(
              tooltip: 'Eliminar selecciÃƒÆ’Ã‚Â³n',
              onPressed: () async {
                final ok = await _confirmDelete(context, count: _sel.length);
                if (!mounted) return;
                if (ok) await _deleteSelected();
              },
              icon: const Icon(Icons.delete),
            ),
            IconButton(
              tooltip: 'Salir selecciÃƒÆ’Ã‚Â³n',
              onPressed: () => setState(() {
                _sel.clear();
                _selectionMode = false;
              }),
...
/// ConfirmaciÃƒÆ’Ã‚Â³n de borrado reutilizable (DRY)
Future<bool> _confirmDelete(BuildContext context, {int count = 1}) async {
  final ok = await showDialog<bool>(
    ...
    builder: (_) => AlertDialog(
      ...
      content: Text(
        count == 1 ? 'Ãƒâ€šÃ‚ÂEliminar esta mediciÃƒÆ’Ã‚Â³n?' : 'Ãƒâ€šÃ‚ÂEliminar $count elementos?',
      ),
      actions: [
...
List<Measurement> _filterAndSort(_FilterArgs a) {
  ...
  out.sort((x, y) {
    final c = a.sortByDate
        ? _safeDateCompare(x.date, y.date)
