...
// Repo de ALMACENAMIENTO (JSON local)
import '../repositories/measurement_repository.dart' as storage_repo;
import '../repositories/local_json_measurement_repository.dart';

/// Repo por planilla (LOCAL JSON)
final storageRepoProvider =
Provider.family<storage_repo.MeasurementRepository, String>(
      (ref, sheetId) => LocalJsonMeasurementRepository(sheetId),
);
...
class MeasurementAsyncNotifier
  ...
  storage_repo.MeasurementRepository get _repo =>
      ref.read(storageRepoProvider(_sheetId));

  @override
  Future<List<Measurement>> build(String sheetId) async {
    _sheetId = sheetId;
  ...
  int _safeDateCompare(DateTime? a, DateTime? b) {
  ...
  int _findIndex(Measurement m, List<Measurement> list) {
    ...
    return list.indexWhere(
            (e) => e.progresiva == m.progresiva && _sameDay(e.date, m.date));
  }
    ...
    next.sort((a, b) {
      ...
      switch (column) {
        ...
        case MeasurementColumn.date:
          r = _safeDateCompare(a.date, b.date);
          break;
        default:
          r = 0;
      }
...
abstract class EditCommand {
  ...
  Future<List<Measurement>> save(storage_repo.MeasurementRepository repo,
      List<Measurement> optimistic);
  Future<List<Measurement>> unsave(storage_repo.MeasurementRepository repo,
      List<Measurement> optimistic);
}
...
class SetAllCommand extends EditCommand {
  ...
  @override
  Future<List<Measurement>> save(storage_repo.MeasurementRepository repo,
      List<Measurement> optimistic) async {
    await repo.saveAll(optimistic);
    return optimistic;
  }
...
class AddRowCommand extends EditCommand {
  ...
  @override
  List<Measurement> execute(List<Measurement> list) {
    final next = List<Measurement>.from(list)..insert(index, item);
    return next;
  }

  @override
  List<Measurement> unexecute(List<Measurement> list) {
    final next = List<Measurement>.from(list)..removeAt(index);
    return next;
  }
...
class UpdateRowCommand extends EditCommand {
  ...
  @override
  List<Measurement> execute(List<Measurement> list) {
    final next = List<Measurement>.from(list)..[index] = newItem;
    return next;
  }

  @override
  List<Measurement> unexecute(List<Measurement> list) {
    final next = List<Measurement>.from(list)..[index] = oldItem;
    return next;
  }
  ...
  Future<List<Measurement>> unsave(storage_repo.MeasurementRepository repo,
      List<Measurement> optimistic) async {
    final prev = List<Measurement>.from(optimistic)..[index] = oldItem;
    await repo.saveAll(prev);
    return prev;
  }
}
