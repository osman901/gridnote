...
class _MeasurementRowEditorState extends State<MeasurementRowEditor> {
  ...
  void initState() {
    ...
    // Si en tu modelo 'date' es nullable, usamos "hoy" como fallback.
    _date = (m as dynamic).date is DateTime? ? (m as dynamic).date ?? DateTime.now() : m.date;
  }
  ...
  Future<void> _fillGPS() async {
    try {
      ...
      if (perm == LocationPermission.denied) {
        ...
        if (perm == LocationPermission.denied ||
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Permiso de ubicaciÃƒÆ’Ã‚Â³n denegado.')),
          );
          return;
        }
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No se pudo obtener la ubicaciÃƒÆ’Ã‚Â³n. Verifique el GPS.'),
  ...
  void _save() {
    ...
    final updated = m.copyWith(
      ...
      ohm1m: _toDouble(_ohm1Ctrl.text) ?? m.ohm1m,
      ohm3m: _toDouble(_ohm3Ctrl.text) ?? m.ohm3m,
                ...
                children: [
                  ...
                  IconButton(
                    ...
                    onPressed: widget.onDuplicate == null
                        ? null
                        : () {
                      // Creamos una nueva Measurement SIN id (queda nulo por omisiÃƒÆ’Ã‚Â³n)
                      final dup = Measurement(
                ...
                children: [
                  Expanded(
                    child: TextFormField(
                      ...
                      textInputAction: TextInputAction.next,
                      decoration: const InputDecoration(labelText: '1 m O'),
                      validator: (v) {
                        final t = (v ?? '').trim();
                        if (t.isEmpty) return null;
                        return _toDouble(t) == null ? 'Valor invÃƒÆ’Ã‚Â¡lido' : null;
                      },
                    ),
                  ),
                  ...
                  Expanded(
                    child: TextFormField(
                      ...
                      inputFormatters: [_numFmt],
                      textInputAction: TextInputAction.next,
                      decoration: const InputDecoration(labelText: '3 m O'),
                      validator: (v) {
                        final t = (v ?? '').trim();
                        if (t.isEmpty) return null;
                        return _toDouble(t) == null ? 'Valor invÃƒÆ’Ã‚Â¡lido' : null;
                ...
                children: [
                  Expanded(
                    child: TextFormField(
                      ...
                      validator: (v) {
                        ...
                        if (val == null) return 'Valor invÃƒÆ’Ã‚Â¡lido';
                        if (val < -90 || val > 90) {
                  ...
                  Expanded(
                    child: TextFormField(
                      ...
                      onEditingComplete: _save, // guarda al tocar ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œListoÃƒÂ¢Ã¢â€šÂ¬Ã‚Â
