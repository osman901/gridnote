// Gridnote · SmartHomeMenu · 2025-09
// Galería ágil con precache + placeholders, sin deps extra.
// Usa SmartTurbo para IA de prefetch.
// Requiere: smart_turbo.dart
...
class SmartHomeMenu extends StatefulWidget {
  const SmartHomeMenu({
    super.key,
    required this.theme,
    required this.photosLoader, // pasá tu loader (ej: () => PhotoStore.recentGlobal(120))
    this.onOpenPhoto,
    this.maxItems = 120,
  });
...
class _SmartHomeMenuState extends State<SmartHomeMenu> {
  ...
  Future<void> _load() async {
    try {
      ...
      // Precache post-frame (no bloquea build)
...
class _Header extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      ...
      Text('Galería reciente', style: TextStyle(fontWeight: FontWeight.w700, color: t.text)),
      const Spacer(),
...
class _ShimmerTileState extends State<_ShimmerTile>
  ...
  Widget build(BuildContext context) {
    return FadeTransition(
      ...
      child: ColoredBox(color: widget.color.withOpacity(.25)),
    );
  }
}
...
class _Bounce extends ScrollBehavior {
  const _Bounce();
  @override
