...
/// Almacenamiento local en **JSON** por planilla (1 archivo por sheet).
class LocalMeasurementRepository implements MeasurementStorage {
  ...
  Future<Measurement> add(Measurement item) async {
    ...
    final maxId = all.fold<int>(
      -1,
          (a, b) => (b.id ?? -1) > a ? (b.id ?? -1) : a,
    );
