...
class _SheetsHubScreenState extends State<SheetsHubScreen> {
  ...
  /// Convierte una lista retornada por el registry (puede ser `List<SheetMeta>` o `List<Sheet>`)
  /// a `List<SheetMeta>` de forma segura.
  List<SheetMeta> _toMetaList(dynamic raw) {
    if (raw is List<SheetMeta>) {
      return List<SheetMeta>.from(raw);
    }
    if (raw is List) {
      return raw.map<SheetMeta>((e) {
        ...
        final nameStr = d.name?.toString() ?? '(sin nombre)';
        ...
        final updated = _parseDate(d.updatedAt ?? d.createdAt);
  ...
  List<SheetMeta> _byDay(DateTime day) {
    ...
    return _all.where((m) => _dateOnly(m.createdAt) == only).toList()
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        // Normal tabs
            : TabBarView(
          children: [
            _ResultsList(items: _all, theme: t, onOpen: _openSheet),
            _CalendarView(
              theme: t,
              month: _visibleMonth,
              ...
              bottom: _DayList(
                day: _selectedDay,
                items: _selectedDay == null ? const [] : _byDay(_selectedDay!),
                theme: t,
                onOpen: _openSheet,
              ),
            ),
...
class _ResultsList extends StatelessWidget {
  const _ResultsList({required this.items, required this.theme, required this.onOpen});
    ...
    for (var d = 1; d <= daysInMonth; d++) {
      ...
      cells.add(
        InkWell(
          ...
          child: Container(
            decoration: BoxDecoration(
              color: isSelected ? theme.accent.withValues(alpha: .25) : null,
              borderRadius: BorderRadius.circular(10),
            ),
            ...
            child: Column(
              ...
              children: [
                Text('$d', style: TextStyle(color: theme.text)),
...
class _DayList extends StatelessWidget {
  const _DayList(
      {required this.day, required this.items, required this.theme, required this.onOpen});
  final DateTime? day;
  final List<SheetMeta> items;
  final GridnoteTheme theme;
  final void Function(SheetMeta) onOpen;
  ...
  Widget build(BuildContext context) {
    if (day == null) {
      return Center(
        child: Text('Elegí un día', style: TextStyle(color: theme.text.withValues(alpha: .7))),
      );
    }
    if (items.isEmpty) {
      return Center(
