...
class _EditMeasurementSheetState extends State<EditMeasurementSheet> {
  ...
  bool _validate() {
    ...
    if (_ohm1.text.trim().isNotEmpty && _parseNum(_ohm1.text) == null) _errOhm1 = 'Número inválido';
    if (_ohm3.text.trim().isNotEmpty && _parseNum(_ohm3.text) == null) _errOhm3 = 'Número inválido';
    final lat = _parseNum(_lat.text);
    final lng = _parseNum(_lng.text);
    if (_lat.text.trim().isNotEmpty && (lat == null || lat < -90 || lat > 90)) _errLat = 'Lat inválida';
    if (_lng.text.trim().isNotEmpty && (lng == null || lng < -180 || lng > 180)) _errLng = 'Lng inválida';
    setState(() {});
  ...
  Future<void> _useCurrentLocation() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Ubicación capturada')),
      );
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  const Expanded(child: Text('Editar medición', style: TextStyle(fontWeight: FontWeight.bold))),
                ...
                children: [
                  Expanded(
                    child: TextField(
                      ...
                      decoration: InputDecoration(labelText: '1m (Ω)', errorText: _errOhm1),
                  ...
                  Expanded(
                    child: TextField(
                      controller: _ohm3,
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                      decoration: InputDecoration(labelText: '3m (Ω)', errorText: _errOhm3),
                      onChanged: (_) => _validate(),
                    ),
                  ),
