...
class SecureStore {
  ...
  // Versión actual del formato cifrado
  ...
  // Lee, decide versión, descifra, y MIGRA a GN2 si venía en GN1
  Future<Uint8List?> readDecryptedFile(File file) async {
    ...
    try {
      ...
      // migración: si era GN1 -> reescribir como GN2
      if (header != _kHeaderCurrent) {
        try {
          await writeEncryptedFile(file, plain);
        } catch (e, st) {
          // no bloquea lectura si falla re-escritura
          await ErrorReport.I.recordError(e, st, hint: 're-encrypt(GN1→GN2)');
        }
      }
      return plain;
